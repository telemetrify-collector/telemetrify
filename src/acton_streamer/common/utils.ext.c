...
void acton_streamerQ_commonQ_utilsQ___ext_init__() {
    // NOP
}
...
acton_streamerQ_commonQ_utilsQ_IPv4Address acton_streamerQ_commonQ_utilsQ_IPv4AddressD_try_parse (B_str s) {
    ...
    if (r == 0) {
        return acton_streamerQ_commonQ_utilsQ_IPv4AddressG_new($NEWTUPLE(4,
            to$int(addr.s_addr >> 24 & 0xff), to$int(addr.s_addr >> 16 & 0xff), to$int(addr.s_addr >> 8 & 0xff), to$int(addr.s_addr & 0xff)));
    }
    return B_None;
}

acton_streamerQ_commonQ_utilsQ_IPv6Address acton_streamerQ_commonQ_utilsQ_IPv6AddressD_try_parse (B_str s) {
    struct in6_addr addr;

    int r = inet_pton(AF_INET6, fromB_str(s), &addr);
    if (r == 0) {
        return acton_streamerQ_commonQ_utilsQ_IPv6AddressG_new($NEWTUPLE(8,
