...
B_bytes prometheusQ_pack(B_list metric_data) {
	...
	for (int j = 0; j < labels.length; ++j) {
	    ...
	    new_label->name = ((B_str)label.components[0]).str;
	    new_label->value = ((B_str)label.components[1]).str;
	...
	Sample **samples_pb = calloc(samples.length, sizeof(Samples*));
